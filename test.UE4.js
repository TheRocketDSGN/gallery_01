...
var Module = {
	...
	UE4_indexedDBVersion: 202208041450, // Bump this number to invalidate existing IDB storages in browsers.
};
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('https://dl.dropboxusercontent.com/s/uus03s4f4supmvw/UE4Game.wasm?dl=0'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/TheRocketDSGN/gallery_01/ac7f01d0f9e08c28f4076914f81fe81c62b54424/UE4Game.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/TheRocketDSGN/gallery_01/16a214943be313742b4749ac6a8d8a7339ddd429/test.data.js'));
		var utilityJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/TheRocketDSGN/gallery_01/002207bd7a374e06336417403b45300d0801621d/Utility.js')).then(addScriptToDom);
		var dataDownload =
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('https://dl.dropboxusercontent.com/s/4ydrhptls6adsi3/test.data?dl=0'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('https://dl.dropboxusercontent.com/s/4ydrhptls6adsi3/test.data?dl=0')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
